...
<div class="bjui-pageContent">
    <table data-toggle="tablefixed" data-width="100%" data-nowrap="true">
    	...
    	<tr>
    		...
    		<td>${list.vegnum}</td>
    		<td>${list.vegprice}</td>
    		<td>${list.vegallprice}</td>
    		<td>${list.shouname}</td>
    		<td>${list.shounumber}</td>
    		<td>${list.shouaddress}</td>
    		<td>${list.buy_zhuangtai}</td>
    		<td>
    			<a href="<%=request.getContextPath()%>/vegorder/edit.do?id=${list.id}" class="btn btn-default" data-toggle="dialog" data-width="1000" data-height="600" data-id="dialog-mask" data-mask="true">编辑</a>
    			<a href="<%=request.getContextPath()%>/vegorder/delete.do?id=${list.id}" class="btn btn-red" data-toggle="doajax" data-confirm-msg="确定要删除吗？">删除</a>
				<a href="<%=request.getContextPath()%>/vegorder/process.do?id=${list.id}" class="btn btn-default" data-toggle="doajax" data-confirm-msg="确定要执行吗？">确认</a>
    		</td>
    	</tr>
    	</c:forEach>
    	</tbody>
